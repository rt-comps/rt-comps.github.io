...
try {
    ...
    switch (true) {
        // If no parameters then set use dafault paths and set stage type to 1 (fall through)
    ...
    const compList = paramList.map(comp => {
        ...
        if (pathList.indexOf(comp) > -1) return comp
                ...
                case 'mjs':
                    {
                        ...
                        const contents = await fs_readFile(`${srcPath}/${file}`, 'utf8');
                ...
                case 'htm':
                    {
                        ...
                        if (flags.get(stgType) % 2 === 1) return fs_copyFile(`${srcPath}/${file}`, `${dstPath}/${file}`);
                        // html-minifier is async so need to handle promise
                        const contents = await mini(await fs_readFile(`${srcPath}/${file}`, 'utf8'), miniOpt);

...
customElements.define(compName,
  class extends rtBC.RTBaseClass {
    ...
    constructor() {
      ...
      // this.#_sR.querySelector('#container').style.height = attributes['form-height'] ? attributes['form-height'].value : '600px';
      ...
      this.setAttribute('id', 'eventBus');

      //### Event Listeners
      const menuNode = this.#_sR.querySelector('#menu-container');
      //___ updatemenu - Respond to product choice 
      menuNode.addEventListener('updatemenu', (e) => this.updateData(e));
      //___ updatecount - Respond to +/- presses when choosing items
      menuNode.addEventListener('updatecount', (e) => this.displayDetailButton(e));
      //___ cartmod - Respond to +/- presses to items in cart
      this.#_sR.querySelector('#cart')
        .addEventListener('cartmod', (e) => this.modifyCart(e));
      //___ add-items_click - Add the currently selected items to the cart
      this.#_sR.querySelector('#add-but')
        .addEventListener('click', () => this.addToCart());
      //___ place-order_click - Send completed order to out of form
      this.#_sR.querySelector('#further-but')
        .addEventListener('click', () => this.continueOrder());
      //___ recover-order_click - Fill cart with the items from the last order
      this.#_sR.querySelector('#recover-but')
        .addEventListener('click', () => this.recoverOrder());
      this.#_sR.querySelector('#sub-but').addEventListener('click', () => this.#_sR.querySelector('form').requestSubmit());
      this.#_sR.querySelector('form').addEventListener('submit', (e) => this.validate(e));
    ...
    initialiseMenu() {
      ...
      // Move user's form to Shadow DOM
      const srcNode = this.querySelector('div[slot="user-details"]');
      const dstNode = this.shadowRoot.querySelector('form[id="details-form"]');
      dstNode.append(...srcNode.children);
    }
    ...
    displayDetailButton(e) {
      ...
      // Get button node
      const buttonNode = this.shadowRoot.querySelector('#details #add-but');
    ...
    displayCartButtons(fiddle = false) {
      ...
      let newBut;
      switch (true) {
        case (this.querySelectorAll('line-item[slot="cart"][count]').length === 0 || localStorage.getItem('lastOrder') || fiddle):
          newBut = 'last';
          break;
    ...
    //--- dispatchOrder
    // Dispatch the current order in an event and reset the form
    dispatchOrder() {
      // Reset form
    ...
    recoverOrder() {
      ...
      localStorage.setItem('currentOrder', localStorage.getItem('lastOrder'))
      // Rebuild cart
      this.updateCart();
    }
    ...
    validate(e) {
      ...
      if (form) {
        ...
        //        for (const [key, value] of formValues.entries()) console.log(key, value);
      }
      else console.warn('Form not found');

      // let valid = true;
      // const reqNodes = [...this.querySelectorAll('form-field[required]')]
      // for (const el in reqNodes) {
      //   console.log(reqNodes[el]);
      //   if (!reqNodes[el].shadowRoot.querySelector('input').value) {
      //     console.log(`Could not find value for ${reqNodes[el].getAttribute('name')}`);
      //     break;
      //   }
      // }
